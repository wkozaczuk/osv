...
static efi_status_t exit_efi_boot_services(struct efi_boot_table *boot, efi_handle_t handle, struct efi_simple_text_output_protocol* out)
{
	...
	uint64_t desc_num = ((uint64_t)mmap_size) / desc_size;
	    ...
	    if (type != EFI_LOADER_CODE && type != EFI_LOADER_DATA && type != EFI_BOOT_SERVICES_CODE && type != EFI_BOOT_SERVICES_CODE && type != EFI_CONVENTIAL_MEMORY) continue;
	    u16snprintf(buffer, 512, "Memory descriptor: type:%u, start:%p, pages:%u\n", type, mmap[i].physical_start, mmap[i].pages);
	    out->output_string(out, buffer);
	}*/
	uint16_t msg[] = u"-------------------------\n";
...
#define KERNEL_MEMORY_SIZE 0x6c0000 //0x6b0e78 Rounded up to the align 0x10000
#define KERNEL_FILE_SIZE   0x6220a4
	...
	efi_handle_t handle, struct efi_system_table *system)
{
	...
	/* disable MMU */
        ...
        asm volatile ("msr SCTLR_EL1, %0;"
                      "isb":: "r" (sctlr));
	(*entry)(0, 0, 0, kernel_phys_start, (uint64_t)cmdline, 0x40000000);
