...
static void test(params& p, bool pinned)
{
    ...
    for(unsigned i = 0; i < p.readers; i++) {
        threads[i] = sched::thread::make([i, &the_lock, &p, data] {
            reader_thread(i, the_lock, p.iterations, data, p.data_len, p.sleep_before);
        }, pinned ? sched::thread::attr().pin(sched::cpus[i % sched::cpus.size()]) : sched::thread::attr());
    }

    for(unsigned i = 0; i < p.writers; i++) {
        threads[p.readers + i] = sched::thread::make([i, &the_lock, &p, data] {
            writer_thread(i, the_lock, p.iterations, data, p.data_len, i + 1, p.writer_loops, p.writer_sleep, p.sleep_before);
        }, pinned ? sched::thread::attr().pin(sched::cpus[i % sched::cpus.size()]) : sched::thread::attr());
    }
