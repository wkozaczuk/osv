...
bool rwlock::try_rlock()
{
    if (_readers < WRITE_INDICATOR) {
        ...
        unsigned prev_readers = readers->load(std::memory_order_acquire); //TODO: Can it be a non-atomic read?
...
void rwlock::wlock()
{
    ...
    std::atomic<unsigned> *readers = reinterpret_cast<std::atomic<unsigned>*>(&_readers);
    uint32_t prev_readers = readers->fetch_or(WRITE_INDICATOR, std::memory_order_acq_rel);
    ...
    for (int i = 0; i < 2; i++ ) { //TODO: Explain why 2
        if ((prev_readers & READER_MASK) == 0 && (prev_readers & LOCK_INDICATOR) == 0) {
            if (readers->compare_exchange_weak(prev_readers, WRITE_INDICATOR | LOCK_INDICATOR | (prev_readers & WAITING_READERS_MASK), std::memory_order_acq_rel)) {
                // we've won the race
                return;
...
void rwlock::downgrade()
{
    ...
    uint32_t prev_readers = readers->load(std::memory_order_acquire);
    while(true) {
        uint32_t next_readers = (prev_readers & ~(LOCK_INDICATOR | WRITE_INDICATOR)) + READER_LOCK_INC;
