...
bool rwlock::wake_read_waiters(bool lock, read_waiter *skip)
{
    //Make sure we are the only consumer of _read_waiters
    std::atomic<bool> *waking_waiters = reinterpret_cast<std::atomic<bool>*>(&_waking_waiters);
    while(true) {
        bool waking = false;
        if (waking_waiters->compare_exchange_weak(waking, true)) {
            break;
        }
    }
    //Wake
...
void rwlock::rlock() {
    ...
    while (true) {
        ...
        if (retry++ > RETRY_THRESHOLD) {
            ...
            tmr.set(now + 1_ms);
            thread::wait_until( [this, &waiter, &tmr] { 
                return waiter.woken || tmr.expired();
            });
            ...
            if (waiter.woken) {
                //We were popped from the queue
                if (waiter.locked) {
                    return;
                }
            } else {
                //Remove all read waiters including us (maybe in vain) if we were not woken by a writer
                if (!wake_read_waiters(false, &waiter)) {
                    //Waiter not found -> woken
                    if (waiter.locked) {
                        return;
                    } //otherwise woken by rlock() -> so let us try again
                } // Waiter found -> NOT woken -> let us try again
            }
        }
    }
}
