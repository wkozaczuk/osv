...
void object::relocate_pltgot(bool dlopen)
{
...
void* __tls_setup(module_and_offset* mao)
{
...
extern "C" OSV_LD_LINUX_x86_64_API
void* __tls_desc_dynamic(module_and_offset* mao)
{
    u64 *dtv = (u64*)sched::thread::current()->get_dtv();
    void *tls = (void*)dtv[mao->module];
    if (!tls) {
        tls = __tls_setup(mao);
    }
    return (void*)(tls + mao->offset);
}

extern "C" OSV_LD_LINUX_x86_64_API
void* __tls_get_addr(module_and_offset* mao)
{
    //debugf("__tls_get_addr: module:%u, offset:%u\n", mao->module, mao->offset);
    //sched::fpu_lock fpu;
    //WITH_LOCK(fpu) {
    ...
    if (tls) {
        ...
        object *obj = get_program()->tls_object(mao->module);
        assert(mao->module == obj->module_index());
        auto tls = obj->setup_tls();
        //debugf("__tls_get_addr: tls:%p\n", tls);
        debugf("[tid:%d] __tls_get_addr: module:%u, offset:%u, tls:%p\n", sched::thread::current()->id(), mao->module, mao->offset, tls);
        return tls + mao->offset;
    }//}
}
