...
public:
    ...
    inline void wake() {
        t.load(std::memory_order_acquire)->wake_with_from_mutex([&] { t.store(nullptr, std::memory_order_release); });
    }
...
struct wait_record : public waiter {
    ...
    void wake_lock(mutex* mtx) { t.load(std::memory_order_acquire)->wake_lock(mtx, this); }
};
